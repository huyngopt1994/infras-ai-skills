...
1. Intake the request into a work item: create `docs/infras-kit/work-items/<nnn>-<slug>/` with `infra-kit.workflow`.
...
- `infra-kit.workflow`: creates the work-item folder and provides the lightweight spec/plan/tasks templates.
...
- `infra-kit.domain.github`: implement/review repository governance and GitHub Actions workflows.
- `infra-kit.workflow.audit`: pre-merge/release audit for least privilege, release safety, OWASP exposure, and well-architected alignment.
